...
(define tests
  (test-suite
   ...
   (check-equal? (list-nth-mod (list 0 1 2 3 4) 7) 2 "list-nth-mod test 2")
   ...
   (check-equal? (stream-for-n-steps dan-then-dog 5) (list "dan.jpg" "dog.jpg" "dan.jpg" "dog.jpg" "dan.jpg") "dan-then-dog test 2")
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero nats) 3) (list (cons 0 1) (cons 0 2) (cons 0 3)) "stream-add-zero test 3")

   ; cycle-lists test
   (check-equal? (stream-for-n-steps (cycle-lists (list 1 2 3) (list "a" "b")) 7)
                 (list (cons 1 "a") (cons 2 "b") (cons 3 "a") (cons 1 "b") (cons 2 "a") (cons 3 "b") (cons 1 "a")) "cycle-lists test")

   ; vector-assoc test
   (check-equal? (vector-assoc 4 (vector (cons 2 1)   (cons 3 1) (cons 4 1) (cons 5 1))) (cons 4 1) "vector-assoc test 1")
   (check-equal? (vector-assoc 5 (vector (cons 2 1)   (cons 3 1) (cons 4 1) (cons 5 1))) (cons 5 1) "vector-assoc test 2")
   (check-equal? (vector-assoc 6 (vector (cons 2 1)   (cons 3 1) (cons 4 1) (cons 5 1)))     #f     "vector-assoc test 3")
   (check-equal? (vector-assoc 3 (vector (cons 2 1) 2 (cons 3 1) (cons 4 1) (cons 5 1))) (cons 3 1) "vector-assoc test 4")

   ; cached-assoc tests
   (check-equal? ((cached-assoc (list (cons 1 2) (cons 3 4)) 3) 3) (cons 3 4) "cached-assoc test 1")
   (check-equal? ((cached-assoc (list (cons 1 2) (cons 3 4)) 3) 4)     #f     "cached-assoc test 2")

   ; while-less test
   (check-equal? (while-less 7 do (begin (set! a (+ a 1)) a)) #t "while-less test 1.1")
   (check-equal? a 7 "while-less test 1.2")
   (check-equal? (while-less 7 do (begin (set! a (+ a 1)) a)) #t "while-less test 2.1")
   (check-equal? a 8 "while-less test 2.2")
   ))
